// FTP (fuckthepopulation) Restock Monitor — cloned from palace-monitor.js
// Uses Shopify Storefront GraphQL API (no auth required)
// Single region: US webstore
...
const WEBHOOKS = {
  US: process.env.FTP_WEBHOOK_US || '',
};

const REGIONS = {
  US: { label: 'FTP', flag: '🔥', baseUrl: 'https://fuckthepopulation.com', currency: 'USD', webhookKey: 'US' },
};
...
const SNAPSHOT_FILE     = process.env.SNAPSHOT_PATH || 'ftp-snapshot.json';
const RESALE_CACHE_FILE = process.env.RESALE_CACHE_PATH || 'ftp-resale-cache.json';
...
async function sendAlert(region, product, variants, isNew) {
  ...
  const embed = {
    ...
    fields,
    footer: { text: `FTP Monitor | ${region.label} | ${timeStr}` },
...
async function pollCycle() {
  ...
  if (!activeWebhooks.length) {
    console.error('No webhooks configured! Set FTP_WEBHOOK_US as env var.');
    process.exit(1);
  }
...
async function main() {
  console.log('🔥 FTP Monitor starting...');
  loadSnapshot();
  loadResaleCache();
  await refreshExchangeRates();
